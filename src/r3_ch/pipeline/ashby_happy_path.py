...
from r3_ch.adapters.opportunity_normalizer import extract_ashby_location, normalize_ashby_job
from r3_ch.ats.ashby_client import AshbyClient
from r3_ch.config import (
    ASHBY_ACTIVE_SLUGS_PATH,
    ASHBY_INVALID_SLUGS_PATH,
    ASHBY_LAST_RUN_PATH,
    ASHBY_MAX_CONCURRENCY,
    ASHBY_PROCESSED_SLUGS_PATH,
    ASHBY_RATE_LIMIT_PER_SEC,
    ASHBY_SLUGS_PATH,
    ASHBY_STATE_DIR,
    OUTPUT_ASHBY_MASTER_MATCHES_PATH,
    OUTPUT_ASHBY_MATCHES_PATH,
)
...
def _build_state_store() -> GreenhouseSlugStateStore:
    return GreenhouseSlugStateStore(
        source_slugs_path=ASHBY_SLUGS_PATH,
        active_slugs_path=ASHBY_ACTIVE_SLUGS_PATH,
        invalid_slugs_path=ASHBY_INVALID_SLUGS_PATH,
        processed_slugs_path=ASHBY_PROCESSED_SLUGS_PATH,
        last_run_path=ASHBY_LAST_RUN_PATH,
    )


def _extract_ashby_content(job: dict[str, Any]) -> str:
...
async def _process_slug(
    ...
    for job in fetch_result.jobs:
        ...
        content = _extract_ashby_content(job)
...
async def run_ashby_happy_path(
    limit: int,
    target_location: str,
    max_concurrency: int = ASHBY_MAX_CONCURRENCY,
    rate_limit_per_sec: float = ASHBY_RATE_LIMIT_PER_SEC,
) -> dict[str, Any]:
    ...
    logger.info(
        "Ashby run config: limit=%s concurrency=%s rate_limit=%s/s source=%s state_dir=%s",
        limit,
        max_concurrency,
        rate_limit_per_sec,
        ASHBY_SLUGS_PATH,
    ...
    master_store = GreenhouseMasterStore(OUTPUT_ASHBY_MASTER_MATCHES_PATH)
    ...
    OUTPUT_ASHBY_MATCHES_PATH.parent.mkdir(parents=True, exist_ok=True)
    with OUTPUT_ASHBY_MATCHES_PATH.open("w", encoding="utf-8") as file:
        for record in matches:
            file.write(json.dumps(record, ensure_ascii=False) + "\n")

    logger.info("Ashby funnel: %s", json.dumps(funnel, ensure_ascii=False))
    return {
        "funnel": funnel,
        "output_path": str(OUTPUT_ASHBY_MATCHES_PATH),
